...
# #         log_message(f"[SERVER] Receiving file of size {total_size}")

# #         received_bytes = 0
# #         with open(f"received_{int(time.time())}.bin", "wb") as out_file:
# #             while received_bytes < total_size:
# #                 header = conn.recv(PACKET_HEADER_SIZE)
# #                 if not header or len(header) < PACKET_HEADER_SIZE:
# #                     break
# #                 seq_num = int.from_bytes(header[:4], 'big')
# #                 data_len = int.from_bytes(header[4:], 'big')
# #                 if seq_num == 0xFFFFFFFF:
# #                     log_message("[SERVER] End of file signal received")
# #                     break
# #                 if data_len == 0:
# #                     continue
# #                 data = b''
...
# #                 conn.sendall(seq_num.to_bytes(4, 'big'))

# #         log_message(f"[SERVER] Finished receiving file of size {received_bytes}")

# #         # ✅ Receive client log (new addition)
# #         log_size_data = conn.recv(16)
# #         if log_size_data:
# #             log_size = int(log_size_data.decode().strip())
# #             if log_size > 0:
# #                 log_data = b''
# #                 while len(log_data) < log_size:
# #                     log_data += conn.recv(min(BUFFER_SIZE, log_size - len(log_data)))

# #                 client_log_file = os.path.join(LOG_DIR, f"client_{int(time.time())}.log")
# #                 with open(client_log_file, "wb") as f:
# #                     f.write(log_data)
# #                 log_message(f"[SERVER] Saved client log to {client_log_file}")
...
#             print(f"[SSL ERROR] {e}")
#     return  # to prevent old accept loop
